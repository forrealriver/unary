...
void Fram::OnBnClickedButton1()
{
	...
	sFilePath=browse.BrowseForFile(this->m_hWnd);
	cc_sEdit.SetWindowText(sFilePath);
}

void Fram::OnBnClickedButton2()
{
	// TODO: Add your control notification handler code here
	dFildPath=browse.BrowseFileFor();
	cc_dEdit.SetWindowText(dFildPath);
}
...
void Fram::OnBnClickedOk()
{
	...
	int x=tansControl.TransformFile(sFilePath,dFildPath,sCode,dCode);
	if (x==0){
		CString error=tansControl.GetLastError();
		AfxMessageBox(error);
	}
	else{
