...
{
	m_Handle = CreateFile(sFilePath.c_str(),GENERIC_READ|GENERIC_WRITE, 0, NULL, OPEN_ALWAYS, 0, NULL);
...
long FileOperate::ReadFileByte(long bytePosition)
{
	...
	if(!ReadFile(m_Handle, m_ReadByte + bytePosition , readByteSize, &m_bytesRead, NULL))
	{
		SetLastError("读文件失败");
		log.OutputLog(m_LastError);
		CloseHandle(m_Handle);
...
long FileOperate::ReadFileLine(byte* readLine,long readLineSize)
{
	...
	while (m_FileReadSize != m_FileSize)
	{
		...
		for (;i<m_ReadLen;i++)
		{
			...
			{
				i=i+m_EenLen;
				if(m_ReadPosition == BUFFER_SIZE-1) //byte中的第一行
				{
					lineSize=i+1;
