...
void CFold_NewDlg::OnBnClickedOk()
{
    ...
    //目录遍历
    Find finder;
    CStringArray *fileNames=new CStringArray[1];
    int fileNum = finder.FindFold(sFolderPath, fileNames);
    if(fileNum == 0) return;

    //读取Xml文件
    XmlRead xmlread;
	CStringArray *filterNames=new CStringArray[1];
	...
	while(filenum > f_num)
    {
		CString findfile = sFolderPath + "\\" + fileNames[0].GetAt(f_num);
        f_num++;
        Compare compare;
		...
		if(new_filename[0] != "")
        {
            ...
            Copy copy;
            CreateDirectory(dFolderPath + "\\" + fileNames[0].GetAt(f_num - 1), NULL);
            CString copyfoldto = _T("将文件夹“" + newFullPath + "”复制到了“"
                                    + dFolderPath + "\\" + fileNames[0].GetAt(f_num - 1) + "\\" + new_filename[0] + "”");
            log.OutputLog(copyfoldto);
            int copy_warnning = copy.copyFold(newFullPath, dFolderPath + "\\" + fileNames[0].GetAt(f_num - 1) + "\\" + new_filename[0]);
        }
    }
	//判断并显示获取情况
...
函数名：ResultWarn
功能说明：显示获取结果情况
输入：resultwarning1、resultwarning2 错误标识
输出：void
*******************************************************************/
void CFold_NewDlg::ResultWarn(int resultwarning1, int resultwarning2)
{
    ...
    {
        log.OutputLog(_T("全部获取成功！"));
        AfxMessageBox(_T("全部获取成功"));
    }
    else if(resultwarning1 == 1 && resultwarning2 == 1)
    {
        log.OutputLog(_T("只获取了部分文件夹，查看上面警告和错误"));
        AfxMessageBox(_T("获取了部分，具体见日志"));
    }
    else if(resultwarning1 == 1 && resultwarning2 == 0)
    {
        log.OutputLog(_T("获取失败，查看上面警告和错误"));
        AfxMessageBox(_T("获取失败，具体见日志"));
    }
}
